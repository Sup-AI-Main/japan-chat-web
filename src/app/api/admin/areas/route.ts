...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!body.name_kr) missing.push("name_kr");
    if (missing.length > 0) {
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const result = await updateArea(id as string, data, updated_at as string | undefined);
    return ok(result);
  } catch (err) {
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    const result = await deleteAreaFull(id, confirmed);
