...
> 지역, 카테고리, 골프장, 호텔, 맛집, FAQ, 추가 안내, 상세 필드 등 모든 관리 대상은 DB 기반으로 동적으로 확장할 수 있어야 한다.
...
6. 호텔/골프장/맛집 등 기존 타입에 새로운 상세 필드를 자유롭게 추가할 수 있어야 한다.
7. 기존 상세 필드를 다른 호텔/골프장/맛집에서도 재사용할 수 있어야 한다.
...
호텔/골프장/맛집 등은 공통 Entity 모델을 기반으로 관리한다.
...
관리자가 호텔/골프장/맛집 등의 상세 데이터를 편집할 때 **정해진 필드만 수정할 수 있는 구조로 만들지 않는다.**
...
어느 호텔/골프장/맛집에서도 새로운 필드를 생성할 수 있어야 한다.
...
현재 골프장 질문/호텔 질문/맛집 질문 등을 별도 화면에서 관리하고 있으므로, 이를 표 기반으로 통합한다.
...
지역별 맛집 관리를 관리자 표에서 별도로 쉽게 볼 수 있어야 한다.
